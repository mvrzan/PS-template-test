...
const filterWantedVars = (type, parent) => {
  ...
  for (const varName in varNameMapping) {
    if (
      varNameMapping[varName].type !== type ||
      (parent && resultCache[varNameMapping[varName].parent] !== parent)
    ) {
      continue;
    }
...
export const fetchServerlessEnvironments = (serviceSid) => {
  ...
  if (!serviceSid) {
    console.warn(
      "Serverless service SID missing; unable to fetch its environments"
    );
    return;
  }
  ...
  const serviceEnvironments = execTwilioCli(
    `twilio api:serverless:v1:services:environments:list --service-sid=${serviceSid}`
  );
...
export const fetchServerlessFunctions = (serviceSid) => {
  ...
  if (!serviceSid) {
    console.warn(
      "Serverless service SID missing; unable to fetch its functions"
    );
    return;
  }
  ...
  const serviceFunctions = execTwilioCli(
    `twilio api:serverless:v1:services:functions:list --service-sid=${serviceSid}`
  );
  ...
  for (const workflow of workflows) {
    for (const wanted in wantedWorkflows) {
      ...
      if (
        ...
        (!resultCache[wanted] &&
          isMatch(
            wantedWorkflows[wanted].fallback,
...
export const fetchTwimlApps = () => {
  ...
  const wantedServices = filterWantedVars(type);
  const services = execTwilioCli("twilio api:core:applications:list");
...
export const fetchTaskChannel = (workspaceSid) => {
  const type = "tr-task-channel";
  // If we already fetched these, no need to do it again
  if (fetchedTypes.includes(type)) {
    return;
  }
  ...
  console.log("Fetching Task Channels...");

  const wantedServices = filterWantedVars(type);
  const services = execTwilioCli(
    `twilio api:taskrouter:v1:workspaces:task-channels:list --workspace-sid=${workspaceSid}`
  );
...
export const fetchStudioFlow = () => {
  const type = "studio-flow";
  // If we already fetched these, no need to do it again
  if (fetchedTypes.includes(type)) {
    return;
  }
  ...
  console.log("Fetching Studio Flows...");

  const wantedServices = filterWantedVars(type);
  const services = execTwilioCli("twilio api:studio:v2:flows:list");
